...
- **작업을 시작할 때 항상 `memory/MEMORY.md`를 먼저 읽고 기존 기록을 확인합니다.**
- **사용자가 지적하거나 수정을 요구한 내용은 `memory/feedback-{주제}.md` 파일로 기록하고, `memory/MEMORY.md` 목차에 한 줄로 등록합니다.**

## 폴더 구조
- `docs/`: 설계 문서, 사용법, 참고 자료
- `samples/`: 예제 스크립트, 샘플 데이터
- `agent.md`: 에이전트(자동화 워크플로) 관련 문서
- `memory/`: 작업 중 누적되는 기록 (`MEMORY.md`가 목차, `feedback-{주제}.md`가 개별 항목)
